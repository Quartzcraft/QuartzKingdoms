...
public class Kingdom {
    ...
    public boolean isEnemy(Kingdom relatingKingdom) {
        try {
            java.sql.PreparedStatement s = QuartzKingdoms.DBKing.prepareStatement("SELECT * FROM relationships WHERE kingdom_id=? AND sec_kingdom_id=? AND status=3 AND pending=0;");
            s.setInt(1, this.id);
            s.setInt(2, relatingKingdom.getID());
            ResultSet res = s.executeQuery();

            java.sql.PreparedStatement s1 = QuartzKingdoms.DBKing.prepareStatement("SELECT * FROM relationships WHERE kingdom_id=? AND sec_kingdom_id=? AND status=3 AND pending=0;");
            s1.setInt(1, relatingKingdom.getID());
            s1.setInt(2, this.id);
            ResultSet res1 = s1.executeQuery();
    ...
    public boolean isPendingEnemy(Kingdom relatingKingdom) {
        try {
            java.sql.PreparedStatement s = QuartzKingdoms.DBKing.prepareStatement("SELECT * FROM relationships WHERE kingdom_id=? AND sec_kingdom_id=? AND status=3 AND pending=1;");
            s.setInt(1, this.id);
            s.setInt(2, relatingKingdom.getID());
            ResultSet res = s.executeQuery();

            java.sql.PreparedStatement s1 = QuartzKingdoms.DBKing.prepareStatement("SELECT * FROM relationships WHERE kingdom_id=? AND sec_kingdom_id=? AND status=3 AND pending=1;");
            s1.setInt(1, relatingKingdom.getID());
            s1.setInt(2, this.id);
            ResultSet res1 = s1.executeQuery();
